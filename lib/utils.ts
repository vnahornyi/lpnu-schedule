...
async function getInstitutes() {
    return fetchHtml().then(html => {
        const select = parseAndGetOne(
            html,
            '#edit-departmentparent-abbrname-selective'
        );
...
async function getTimetable(
    studygroup_abbrname_selective = 'All',
...
function parseDay(day) {
    ...
    const contentChildren = day.querySelector(
    ...
    for (let i = 0; i < contentChildren.length; i++) {
        ...
        if (child.classList.contains('stud_schedule')) {
            ...
            if (currentLessonNumber === 0)
                console.warn('Lesson number is 0!', child);
